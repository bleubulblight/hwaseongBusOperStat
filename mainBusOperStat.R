...
for (i in 1:nrow(avail_index)) {
  ...
  line_length <- max(this_data$stationSeq)
  ...
  timespace <- ggplot(data=this_data,mapping=aes(x=timestamp,y=stationSeq,colour=plateNo)) +
    ...
    theme(plot.title = element_text(hjust = 0.5),panel.grid.major.y =element_line(colour="grey")) +
    scale_x_discrete(labels=NULL) +
    scale_y_continuous(limits=c(0,line_length))
